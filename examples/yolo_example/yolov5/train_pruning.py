...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    DG.build_dependency(model, example_inputs=torch.randn([1, 3, opt.imgsz, opt.imgsz]).cuda())

    excluded_layers = list(model.model[-1].modules())
    for m in model.modules():
        if isinstance(m, nn.Conv2d) and m not in excluded_layers:
            pruning_plan = DG.get_pruning_plan(m, tp.prune_conv_out_channel, idxs=strategy(m.weight, amount=opt.amount))
            print(pruning_plan)
