    ...
    while(True):
        if not pruned:
            ...
            with torch.cuda.amp.autocast(True):
                outputs = model(img[:pruning_batch_size])
                loss, _ = compute_loss(outputs, target.cuda()[:pruning_batch_size])
                ...
                # outputs = model(img[:pruning_batch_size].cuda())
                # loss, _ = compute_loss(outputs, target.cuda()[:pruning_batch_size])
                # loss = scaler_crop.scale(loss)
                ...
                # for layer in model.modules():
                #     if hasattr(layer, 'weight') and layer.weight.requires_grad == True:
                #         z += (grad_w[count] * grad_f[count]).sum() * 1e-9
                #         print(z)
                #         count += 1
                # z = scaler_crop.scale(z)
                # z.backward()

            pruner = tp.pruner.GradientPruner(
                model,
                example_inputs,
                crit='SNIP',
            ...
            # progressive_pruning(pruner, model, speed_up=1.5, example_inputs=example_inputs)
