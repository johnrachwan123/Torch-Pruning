...
class DependencyGraph(object):
    ...
    def build_dependency(
        ...
        for m in self.model.modules():
            ...
            if (
                    op_type in prunable_module_types and op_type != ops.OPTYPE.ELEMENTWISE) or m.__class__ in self.CUSTOMIZED_PRUNERS.keys():
        ...
        for p in unwrapped_detected:
            ...
            if pruning_dim is not None:
                unwrapped_parameters.append(UnwrappedParameters(parameters=p,
                                                                pruning_dim=pruning_dim))  # prune the last non-singleton dim by daufault
    ...
    def get_pruning_plan(self, module: nn.Module, pruning_fn: typing.Callable,
                         idxs: typing.Union[list, tuple]) -> Group:
    ...
    def _trace(self, model, example_inputs, forward_fn, output_transform):
        ...
        # import pdb
        # pdb.set_trace()
        hooks = [m.register_forward_hook(_record_grad_fn) for m in model.modules() if
                 (isinstance(m, registered_types) and m not in self.IGNORED_LAYERS)]
